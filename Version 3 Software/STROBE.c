...
//*****************  200 Hz Lights Flashing Speed, 1% Duty Cycle  ************************//
	...
	frequency_high = 0xF9;		// 64702 = 2400Hz.
	frequency_low = 0x7D;
	...
	while(1) {
		...
		switch (process_cycle) {
			...
			case 4:
					...
					// to 100% and the adjacent ones are brightened according to adjacent_brightness and
					// adjacent2_brightness in my_definitions.
			...
			case 5:
					...
					// When the ADC is enabled this function will cycle through checking the two pots on the
			 		...
			 		// and dial2 for the right pot.
					// Only runs if ms_enable is set.
					strobe_adc(run_adc);
					process_cycle = 1;
					break;
