	...
	switch (ms_process) {
		...
		// TEST
						...
						if (zone_check[i] != zone_check_old[i]) {
							...
							if (zone_check[i]) {
								...
								// If tracking motion, turn off adjacent zones when new activity is detected.
								if (ms_tracking) {
								...
								// If not tracking motion, turn on every zone activity is detected.
								} else {
		...
		// TEST
					...
					for (i = 4; i <= 9; i++) {
						...
						} else if (zone_active[i-1] || zone_active[i+1]) {
							zone_brightness_target[i].r = adjacent_brightness;
							zone_brightness_target[i].g = adjacent_brightness;
							zone_brightness_target[i].b = adjacent_brightness;
						} else if (zone_active[i-2] || zone_active[i+2]) {
							zone_brightness_target[i].r = adjacent2_brightness;
							zone_brightness_target[i].g = adjacent2_brightness;
							zone_brightness_target[i].b = adjacent2_brightness;
						} else if (zone_active[i-3] || zone_active[i+3]) {
							zone_brightness_target[i].r = adjacent3_brightness;
							zone_brightness_target[i].g = adjacent3_brightness;
							zone_brightness_target[i].b = adjacent3_brightness;
						} else {
		...
		// TEST
					...
					// New activity finished being acted on.
					new_activity = 0;
			} break;
		...
		case ms_deinit: {
					...
					ms_tracking = 0;		// Turn off motion sensor tracking.
