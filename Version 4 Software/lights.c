...
void strobe_lights(unsigned char lights_process)
{
	...
	switch (lights_process) {
		...
		// Other Mode Initialization.  (1.69 us)
		case led_manual_init: {

							frequency_timer = 1;		// Enable the timers for the lights.
							dutycycle_timer = 1;

							for (i = 1; i <= 12; i++) {	// Set all the brightnesses to 20%.
								zone_brightness_target[i].r = 1;
								zone_brightness_target[i].g = 1;
								zone_brightness_target[i].b = 1;
							}
		...
		case led_ms_deinit: {
							...
							do {
								...
								for (i = 1; i <=12; i++) {
									if (zone_brightness_buffer[i].r != 0) {
										lights_shutdown = 1;
									}
									if (zone_brightness_buffer[i].g != 0) {
										lights_shutdown = 1;
									}
									if (zone_brightness_buffer[i].b != 0) {
										lights_shutdown = 1;
									}
								}
