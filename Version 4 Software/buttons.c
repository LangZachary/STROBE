 ...
 **********************************/	
		...
		case select_menu1b:		switch (*trigger) {
									...
									// ENTER PRESSED //
									case button_enter_pressed:	oled.menu = manual_menu1;
																strobe_oled(oled_menu, 0, 0);
 ...
 **********************************/
		...
		case motion_menu1a_on:	switch (*trigger) {
									...
									case button_enter_pressed:	oled.menu = motion_menu1a_off;
																...
																strobe_lights(led_ms_deinit);			// Deinitialize lights.			
																break;
		...
		case motion_menu1b:		switch (*trigger) {
									...
									case button_enter_pressed:	oled.menu = motion_menu2a;
																strobe_oled(oled_menu, 0, 0);
																strobe_adc(adc_ms_init);							// Enable ADC Start Sequence.
 ...
 **********************************/
		...
		case motion_menu2a:		switch (*trigger) {
									...
									case button_menu_pressed:	if (ms_timer) {
																...
																strobe_oled(oled_menu, 0, 0);
																strobe_adc(adc_ms_deinit);							// Enable ADC Stop Sequence.
 ...
 **********************************/
		...
		// MANUAL MENU 1 //
		case manual_menu1:	switch (*trigger) {
	
									// MENU PRESSED //
									case button_menu_pressed:	oled.menu = select_menu1a;
																strobe_oled(oled_menu, 0, 0);
																strobe_adc(adc_manual_deinit);			// Disable ADC.
																strobe_lights(led_manual_deinit);		// Disable lights.	
																break;

									// ENTER PRESSED //
									case button_enter_pressed:	oled.menu = manual_menu1a_zone;
																strobe_oled(oled_menu, 0, 0);
																strobe_blink(blink_init);
																manual_zone_select_old = manual_zone_select; // Save the current zone.
																break;
		...
		// MANUAL MENU 1a - Zone //
		case manual_menu1a_zone:switch (*trigger) {
	
									// MENU PRESSED //
									case button_menu_pressed:	strobe_blink(blink_deinit);
																oled.menu = manual_menu1a;
																strobe_oled(oled_menu, 0, 0);
																manual_zone_select = manual_zone_select_old; // Reload the old zone.
																strobe_oled(oled_manual_zone, 0, 0);		 // Display the old zone.
																break;
									...
									// UP PRESSED //
									case button_up_pressed:		if (manual_zone_select == 12) {
																	manual_zone_select = 0;
																} else {
																	manual_zone_select++;
																}
																strobe_oled(oled_manual_zone, 0, 0);							
																break;			

									// DOWN PRESSED //
									case button_down_pressed:	if (manual_zone_select == 0) {
																	manual_zone_select = 12;
																} else {
																	manual_zone_select--;
																}
																strobe_oled(oled_manual_zone, 0, 0);	
		...
		case manual_menu1b:	switch (*trigger) {
									...
									// ENTER PRESSED //
									case button_enter_pressed:	oled.menu = manual_menu1b_red;
																strobe_oled(oled_menu, 0, 0);
																strobe_blink(blink_init);				// Initialize Blinking of "Red".
																strobe_adc(adc_manual_init);			// Enable ADC Start Sequence.
																
																// Save the current brightness
																if (manual_zone_select == 0) {
																	for (i = 1; i <=12; i++) {
																		zone_brightness_target_old[i].r = zone_brightness_target[i].r;
																	}
																} else {
																	zone_brightness_target_old[manual_zone_select].r = zone_brightness_target[manual_zone_select].r;
																}
																break;
									...
									// DOWN PRESSED //
									case button_down_pressed:	oled.menu = manual_menu1c;
																strobe_oled(oled_menu, 0, 0);
																break;
								}break;

		// MANUAL MENU 1b - Red //
		case manual_menu1b_red:	switch (*trigger) {
	
									// MENU PRESSED //
									case button_menu_pressed:	strobe_blink(blink_deinit);
																strobe_adc(adc_manual_deinit);				// Enable ADC Stop Sequence.
																oled.menu = manual_menu1b;
																strobe_oled(oled_menu, 0, 0);
																...
																if (manual_zone_select == 0) {
																	...
																	zone_brightness_target[manual_zone_select].r = zone_brightness_target_old[manual_zone_select].r;
																	print_progress_bar(1, 9, zone_brightness_target[manual_zone_select].r);
																}
...
/*
		...
		case manual_new_color:		switch (*trigger) {
										...
										case button_menu_pressed:	if (manual_change_red) {							// If currently changing one of the colors,
																		...
																		strobe_oled(oled_menu, 0, 0);								// erase the temporary brightness values.
