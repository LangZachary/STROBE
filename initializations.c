...
void int_init(void)
{
	...
	T0CONbits.T0PS = 0b000;		// 1:2 Prescale.
	...
	// Blink Timer4 - 4ms - Low Priority //
	T4CONbits.T4OUTPS = 0b1111;	// 1:16 Postscale.
	T4CONbits.T4CKPS = 0b11;	// 1:16 Prescale.
	PIE5bits.TMR4IE = 1;		// Enable Interrupt.
	IPR5bits.TMR4IP = 0;		// Low priority.
	TMR4 = 0x00;				// Clear Timer count.
	PR4 = 0xFA;					// 4ms interrupt.
	blink_timer_flag = 0;		// Reset Interrupt Flag.
	blink_timer = 0;			// Turn Timer4 Off.
	...
	// Stage Timer6 - 4ms - Low Priority //
	T6CONbits.T6OUTPS = 0b1111;	// 1:16 Postscale.
	T6CONbits.T6CKPS = 0b11;	// 1:16 Prescale.
	PIE5bits.TMR6IE = 1;		// Enable Interrupt.
	IPR5bits.TMR6IP = 0;		// Low priority.
	TMR6 = 0x00;				// Clear Timer count.
	PR6 = 0xFA;					// 4ms interrupt.
	stage_timer_flag = 0;		// Reset Interrupt Flag.
	stage_timer = 0;			// Turn Timer4 Off.
	...
	// Strobelight Timer8 - 100us - Low Priority //
	T8CONbits.T8OUTPS = 0b1111;	// 1:16 Postscale.
	T8CONbits.T8CKPS = 0b01;	// 1:4 Prescale.
	PIE5bits.TMR8IE = 1;		// Enable Interrupt.
	IPR5bits.TMR8IP = 0;		// Low priority.
	TMR8 = 0x00;				// Clear Timer count.
	PR8 = 0x19;					// 100us interrupt.
	strobelight_timer_flag = 0;	// Reset Interrupt Flag.
	strobelight_timer = 0;		// Turn Timer8 Off.

	// Load Timer8 - 2ms - Low Priority //
	T10CONbits.T10OUTPS = 0b1111;// 1:16 Postscale.
	T10CONbits.T10CKPS = 0b01;	// 1:16 Prescale.
	PIE5bits.TMR10IE = 1;		// Enable Interrupt.
	IPR5bits.TMR10IP = 0;		// High Priority.
	TMR8 = 0x00;				// Clear Timer count.
	PR8 = 0x7D;					// 2ms interrupt.
	load_timer_flag = 0;		// Reset Interrupt Flag.
	load_timer = 0;				// Turn Timer10 Off.
}
...
void oled_init(void)
{
	...
	// Clear the oled.display.
  	strcpypgm2ram((char *)oled.display[0], "                    ");
  	strcpypgm2ram((char *)oled.display[1], "                    ");
  	strcpypgm2ram((char *)oled.display[2], "                    ");
  	strcpypgm2ram((char *)oled.display[3], "                    ");
